...
class AuthRbac extends \lithium\core\DynamicObject {
	 ...
	 * @return array empty array if access is allowed, array with reasons for denial if denied.
	 */
	public function check($user, $request, array $options = array()) {
		...
		foreach ($this->_roles as $role) {
			...
			switch (true) {
				case ($role['allow'] === false):
				case (!static::_hasRole($role['requesters'], $request, $options)):
				case (is_array($role['allow']) && !static::_parseClosures($role['allow'], $request, $role)):
					$accessable = false;
					break;
				default:
					$accessable = true;
	 ...
	 * @return mixed
	 */
	protected static function _parseClosures(array &$data = array(), $request = null, array &$options = array()) {
		$return = true;
